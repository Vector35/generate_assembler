/* all the assembler functionality */
...
int disasm(uint8_t *data, uint32_t addr, string& result, string& err)
{
	...
	MYLOG("calling cs_disasm(addr=%08X)\n", addr);
...
{
	static int yup = 0;
	//printf("tokenize() was called %d times\n", yup++);
	int rc = -1, n=0;
	char *endptr;
...
int assemble_single(string src, uint32_t addr, uint8_t *result, string& err, int *failures)
{
	...
	MYLOG("assemble_single(\"%s\", 0x%08X, ...)\n", src.c_str(), addr);
	...
	if(toks_src[0].sval[0]=='b' && toks_src[0].sval.back() != 'a' && toks_src.back().type == TT_NUM) {
		MYLOG("pre-adjust for relative branch!\n");
		toks_src.back().ival -= addr;
		printf("final token is now: 0x%08X\n", toks_src.back().ival);
		addr = 0;
	}
...
int split_newlines(string src, vector<string> &lines)
{
	lines.clear();
	...
	for(int i=0; i<src.size(); ) {
		...
		else if(i+1 < src.size() && p[i]=='\x0d' && p[i+1]=='\x0a') {
			if(left < i) {
				lines.push_back(string(p,left,i-left));
			}
